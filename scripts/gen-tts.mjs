...
for (const t of [
  ...
  // 허브 · 과목 선택 안내
  '영어는 곧 만나요', '한글을 배워요', '숫자를 배워요', '영어를 배워요',
  // 보상: 알 부화 · 마법 정원 · 매일 선물상자
  '오늘의 선물이에요!', '아기 친구들을 모두 모았어요', '활짝 폈어요', '씨앗을 심었어요', '정원이 가득 찼어요',
  // 영어 파닉스
  '첫소리 그림을 찾아요',
  // 우리 가족 읽기(개인화) — 인트로만 프리캐시(가족 단어 자체는 커스텀이라 브라우저 음성)
  '우리 가족을 읽어 봐요',
  // 실사 공주 룩
  '참 예뻐요!', '장미 공주', '겨울 공주', '라벤더 공주', '정원 공주', '루비 공주',
  // 아기동물 이름(알 부화)
  '삐약이', '깡총이', '곰곰이', '야옹이', '멍이', '살랑이', '판단이', '뒤뚱이', '어흥이', '유니', '개굴이', '햄찌',
]) add(t)
